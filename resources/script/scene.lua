...
Init = function()
    ...
    model = SceneObject:CreateObject():Model();
    model.mesh:Set("General.Model.env3");
    model.material:Set("General.Material.env");
    --model:GetObject():Transform():Scale(0.1);
    model:GetObject():MeshCollider():SetMesh("General.Model.env3");
    model:GetObject():SetName("environment");
    
    model = SceneObject:CreateObject():Model();
    model.mesh:Set("General.Model.stylized");
    model.material:Set("General.Material.material1");
    model:GetObject():MeshCollider():SetMesh("General.Model.cube");
    model:GetObject():SetName("Secret!");
    model:GetObject():Transform():Translate(0.0, 0, 1.0);
    
    model = SceneObject:CreateObject():Model();
    model.mesh:Set("General.Model.Jog Forward");
    model.material:Set("General.Material.material2");
